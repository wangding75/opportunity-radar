...
"@
    ...
    $queue = docker compose -p $project @composeFiles exec -T postgres psql -U opportunity_radar -d opportunity_radar -Atc "SELECT status || '|' || attempt_count FROM webhook_delivery_queue WHERE alert_event_id = (SELECT id FROM alert_events WHERE event_key = 'docker-synthetic-webhook-event');"
